...
# Structured outputs (json_schema/parse) are only supported on a subset of
# OpenAI-compatible models. Use this helper to decide whether we should attach
# a response_format schema when keyword extraction is requested.
def _supports_structured_outputs(model: str | None, base_url: str | None, use_azure: bool) -> bool:
    if not model:
        return False

    normalized = model.lower()

    # Azure OpenAI deployments map to OpenAI models even if the deployment name
    # itself is arbitrary, so we allow structured outputs when use_azure is set.
    if use_azure:
        return True

    # For non-OpenAI compatible endpoints (e.g., third-party base_url), skip
    # structured outputs to avoid 400 responses for unsupported types.
    ...
    return any(normalized.startswith(prefix) for prefix in structured_prefixes)
...
# noqa  Mi80OmFIVnBZMlhtblk3a3ZiUG1yS002ZG05TE5nPT06NTRhOTYwODA=
    ...
    # Attach structured response schema only when the backend advertises support.
    if keyword_extraction:
        if _supports_structured_outputs(api_model, base_url, use_azure):
            kwargs["response_format"] = GPTKeywordExtractionFormat
        else:
            logger.info(
                "Structured keyword extraction requested, but model/base_url "
                "does not support response_format; falling back to standard generation"
            )
