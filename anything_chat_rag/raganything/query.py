...
# type: ignore  My80OmFIVnBZMlhtblk3a3ZiUG1yS002UjFGdFRRPT06ZTMwMzA2NTA=
    ...
    async def _process_image_paths_for_vlm(self, prompt: str) -> tuple[str, int]:
        ...
        if prompt is None:
            self.logger.warning(
                "VLM enhanced query received empty prompt from backend; skipping image processing"
            )
            return "", 0

        if not isinstance(prompt, str):
            self.logger.debug(
                "Coercing non-string prompt (%s) to string for image processing", type(prompt)
            )
            prompt = str(prompt)
