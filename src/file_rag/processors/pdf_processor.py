...
def extract_pdf_text(
    ...
    try:
        ...
        if settings.ENABLE_PDF_MULTIMODAL:
            # 创建多模态图片解析器 - 动态创建模型以确保使用最新配置
            from file_rag.core.llms import create_image_model

            current_image_model = create_image_model()
            if current_image_model is not None:
                try:
                    image_parser = LLMImageBlobParser(model=current_image_model)
                except Exception as parser_err:
                    logger.warning(
                        f"创建图像解析器失败，将回退为纯文本模式: {parser_err}"
                    )
            else:
                logger.info("图像解析模型不可用，默认使用纯文本解析")

        def _load_documents(enable_multimodal: bool):
            if enable_multimodal and image_parser is not None:
        ...
        use_multimodal = settings.ENABLE_PDF_MULTIMODAL and image_parser is not None

        try:
            documents = _load_documents(use_multimodal)
        except Exception as loader_err:
            if use_multimodal:
                logger.warning(
                    f"PyMuPDF4LLM 多模态解析失败，回退为纯文本模式: {loader_err}"
                )
                documents = _load_documents(False)
            else:
                raise
